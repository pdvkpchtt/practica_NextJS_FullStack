...
import { fetchFollowers } from "../../server/actions/connections/fetchFollowers";
...
const FollowersModal = ({
  ...
  const getRecruters = async (cursor) => {
    ...
    // if (loading) return;
    setLoading(true);
    const data = await fetchFollowers(comapnyId, cursor, input);
    console.log("client followers", data);
    if (cursor.length) {
